    ...
    public function authenticate() {  
        ...
        if ($admin_email != '' && $admin_password != '') 
        {
            ...
            if (count($admin_check) > 0 && $admin_check != 0) 
            {
                ...
                // $data = array('log_by' => 1, 'log_by_id' => $admin_check[0]['admin_id'], 'log_date' => date('Y-m-d'),
                //     'log_time' => date('h:i:s'), 'log_ip' => getHostByName(getHostName()), 'log_status' => 1);

                // $insert = $this->common->insert_data($data, 'activity_logs');
